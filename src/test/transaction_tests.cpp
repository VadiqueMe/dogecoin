...
BOOST_AUTO_TEST_CASE(test_Get)
{
    ...
    BOOST_CHECK_EQUAL( coins.GetValueIn( t1 ), ( 50 + 21 + 22 ) * E6COIN ) ;
}
...
void CreateCreditAndSpend(const CKeyStore& keystore, const CScript& outscript, CTransactionRef& output, CMutableTransaction& input, bool success = true)
{
    ...
    inputm.nVersion = 1;
    inputm.vin.resize(1);
    inputm.vin[0].prevout.hash = output->GetTxHash() ;
...
BOOST_AUTO_TEST_CASE(test_big_witness_transaction) {
    ...
    for ( int i = 0 ; i < 20 ; i ++ )
        scriptcheckthreads.push_back( std::thread( [&]{ scriptcheckqueue.Loop() ; } ) ) ;
...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    t.vin.resize(1);
    t.vin[0].prevout.hash = dummyTransactions[0].GetTxHash() ;
    t.vin[0].prevout.n = 1;
    t.vin[0].scriptSig << std::vector<unsigned char>(65, 0);
    t.vout.resize(1);
    t.vout[0].nValue = 90 * E6COIN ;
    CKey key;
    key.MakeNewKey(true);
