...
Verify(const CScript& scriptSig, const CScript& scriptPubKey, bool fStrict, ScriptError& err)
{
    ...
    txTo.vin[0].prevout.hash = txFrom.GetTxHash() ;
...
BOOST_AUTO_TEST_CASE(AreInputsStandard)
{
    ...
    coins.ModifyCoins( txFrom.GetTxHash() )->FromTx( txFrom, 0 ) ;
    ...
    for ( int i = 0 ; i < 5 ; i ++ )
    {
        ...
        txTo.vin[ i ].prevout.hash = txFrom.GetTxHash() ;
    }
    ...
    txToNonStd1.vin[0].prevout.hash = txFrom.GetTxHash() ;
    ...
    txToNonStd2.vin[0].prevout.hash = txFrom.GetTxHash() ;
