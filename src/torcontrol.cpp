...
bool TorControlConnection::Connect(const std::string &target, const ConnectionCB& _connected, const ConnectionCB&  _disconnected)
{
    ...
    if ( b_conn == nullptr ) return false ;
    ...
    , reconnect_timeout( RECONNECT_TIMEOUT_START )
{
    ...
    if ( pkf.first ) {
        LogPrintf( "tor: read cached private key from %s\n", GetPrivateKeyFile() ) ;
...
{
    if ( reply.code == 250 ) {
        LogPrintf( "tor: add_onion successful\n" ) ;
        ...
        if ( WriteBinaryFile( GetPrivateKeyFile(), private_key ) ) {
            ...
            LogPrintf( "tor: can't write service private key to %s\n", GetPrivateKeyFile() ) ;
        }
        AddLocal( service, LOCAL_MANUAL ) ; // ... and leave onion connection open
    } else if ( reply.code == 510 ) { // 510 Unrecognized command
        LogPrintf( "tor: add_onion failed with 510 \"unrecognized command\" (you probably need to upgrade Tor)\n" ) ;
    } else {
        LogPrintf( "tor: add_onion failed, error code %d\n", reply.code ) ;
    }
}
...
{
    if ( reply.code == 250 ) {
        LogPrintf( "tor: authentication okay\n" ) ;
        ...
        // and finally now create the service
        if ( private_key.empty() ) // no private key, generate one
            private_key = "NEW:RSA1024" ; // explicitly request RSA1024 - see issue #9214
        // request hidden service, redirect port
        // the 'virtual' port doesn't have to be the same as our internal port,
        //   but this is just a convenient choice
...
{
    if (reply.code == 250) {
        LogPrintf( "tor: SAFECOOKIE authentication challenge okay\n" ) ;
...
void TorController::protocolinfo_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if ( reply.code == 250 /* AUTH */ ) {
        ...
        if ( ! torpassword.empty() ) {
            if ( methods.count( "HASHEDPASSWORD" ) != 0 ) {
                LogPrintf( "tor: using HASHEDPASSWORD authentication\n" ) ;
        ...
        } else if ( methods.count( "SAFECOOKIE" ) != 0 ) {
            // cookie: hexdump -e '32/1 "%02x""\n"'  ~/.tor/control_auth_cookie
            LogPrintf( "tor: using SAFECOOKIE authentication, reading cookie authentication from %s\n", cookiefile ) ;
            ...
            if ( status_cookie.first && status_cookie.second.size() == TOR_COOKIE_SIZE ) {
                ...
                _conn.Command( "AUTHCHALLENGE SAFECOOKIE " + HexStr( clientNonce ),
                                    std::bind( &TorController::authchallenge_cb, this, std::placeholders::_1, std::placeholders::_2 ) ) ;
            } else {
        ...
        } else if ( methods.count( "HASHEDPASSWORD") != 0 ) {
            LogPrintf( "tor: the only supported authentication method is password, but no password provided with -torpassword\n" ) ;
        } else {
...
{
    // try to reconnect and reestablish if we get booted - for example, Tor may be restarting
...
{
    return ( GetDirForData() / "onion_private_key" ).string() ;
}
...
void StartTorControl()
{
    ...
    base = event_base_new() ;
    if ( base == nullptr ) {
        LogPrintf( "tor: can't create event_base\n" ) ;
        return ;
    }

    torControlThread = std::thread( std::bind( &TraceThread< std::function< void() > >, "torcontrol", &TorControlThread ) ) ;
}
