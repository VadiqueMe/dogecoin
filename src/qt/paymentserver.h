...
// This class handles payment requests from clicking on dogecoin: URIs
//
...
public:
    ...
    // which were successfully sent to an already-running process
    // Note: if a payment request is given, SelectParams(MAIN/TESTNET)
    // will be called so we startup in the right mode
    static bool ipcSendCommandLine();

    PaymentServer( QApplication * parent, bool startLocalServer = true ) ;
    ~PaymentServer() ;
    ...
    // Verify that the payment request network matches the current network
...
protected:
    // Constructor registers this on the parent QApplication
    // to receive QEvent::FileOpen and QEvent:Drop events
