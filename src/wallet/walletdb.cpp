...
bool CWalletDB::WriteName( const std::string & address, const std::string & name )
{
    nWalletDBUpdateCounter ++ ;
    return Write( std::make_pair( std::string( "name" ), address ), name ) ;
}

bool CWalletDB::EraseName( const std::string & address )
{
    // This should only be used for sending addresses, never for receiving addresses,
    // receiving addresses must always have an address book entry if they're not change return
    nWalletDBUpdateCounter ++ ;
    return Erase( std::make_pair( std::string( "name" ), address ) ) ;
}

bool CWalletDB::WritePurpose( const std::string & address, const std::string & purpose )
{
    nWalletDBUpdateCounter ++ ;
    return Write( std::make_pair( std::string( "purpose" ), address ), purpose ) ;
}

bool CWalletDB::ErasePurpose( const std::string & address )
{
    nWalletDBUpdateCounter ++ ;
    return Erase( std::make_pair( std::string( "purpose" ), address ) ) ;
}

bool CWalletDB::WriteTx( const CWalletTx & wtx )
{
    nWalletDBUpdateCounter ++ ;
    return Write( std::make_pair( std::string( "tx" ), wtx.GetTxHash() ), wtx ) ;
}
             ...
             CWalletScanState &wss, std::string& strType, std::string& strErr)
{
    try {
        ...
        else if ( strType == "tx" )
        {
            ...
            if ( ! ( CheckTransaction( wtx, state ) && ( wtx.GetTxHash() == hash ) && state.IsValid() ) )
                return false ;
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    if ( wss.nFileVersion < PEER_VERSION ) // Update
...
{
    // Recovery procedure:
    //   rename wallet file appending .backup.timestamp
    //   invoke Salvage with fAggressive=true to get as much data as possible
    //   rewrite salvaged data to fresh wallet file
    //   set -rescan so any missing transactions will be found
    std::string newFilename = strprintf( "%s.backup.%d", filename.c_str(), GetTime() ) ;
    ...
    for ( CDBEnv::KeyValuePair & row : salvagedData )
    {
        if ( fOnlyKeys )
        {
            CDataStream ssKey( row.first, SER_DISK, PEER_VERSION ) ;
            CDataStream ssValue( row.second, SER_DISK, PEER_VERSION ) ;
            std::string strType, strErr ;
            bool fReadOK ;
            {
