...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CBase58Address( CScriptID( redeemScript ) ).ToString() ;
...
{
    assert( mapWallet.count( wtxid ) > 0 ) ;
    CWalletTx & thisTx = mapWallet[ wtxid ] ;
    if ( thisTx.IsCoinBase() ) return ; // coinbases don't spend anything
...
bool CWallet::AddToWallet( const CWalletTx& wtxIn, bool fFlushOnClose )
{
    ...
    if ( fInsertedNew )
    {
        ...
        if ( ! wtxIn.hashUnset() )
        {
            if ( mapBlockIndex.count( wtxIn.hashBlock ) != 0 )
            {
    ...
    if ( ! fInsertedNew )
    {
        ...
        {
            LogPrintf( "AddToWallet: previously abandoned transaction %s is coming back not in a block\n",
                        wtxIn.GetTxHash().ToString() ) ;
            LogPrintf( "AddToWallet: removing tx %s from mempool\n", wtxIn.GetTxHash().GetHex() ) ;
            mempool.removeRecursive( mempool.getTxByHash( wtxIn.GetTxHash() ) ) ;
            return false ;
        }
    ...
    LogPrintf( "AddToWallet %s %s%s\n", wtxIn.GetTxHash().ToString(), fInsertedNew ? "new" : "", fUpdated ? "update" : "" ) ;
    ...
    for ( const CTxIn & txin : wtx.tx->vin ) {
        if (mapWallet.count(txin.prevout.hash)) {
            ...
            if (prevtx.nIndex == -1 && !prevtx.hashUnset()) {
                MarkConflicted( prevtx.hashBlock, wtx.GetTxHash() ) ;
            }
        }
    }
...
{
    {
            ...
            for ( const CTxIn & txin : tx.vin ) {
                ...
                while (range.first != range.second) {
                    if ( range.first->second != tx.GetTxHash() ) {
                        ...
                        MarkConflicted(pIndex->GetBlockSha256Hash(), range.first->second);
                    }
                    range.first++;
                }
            }
        ...
        bool fExisted = mapWallet.count( tx.GetTxHash() ) != 0 ;
...
bool CWallet::AbandonTransaction( const uint256& hashTx )
{
    ...
    while (!todo.empty()) {
        ...
        if (currentconfirm == 0 && !wtx.isAbandoned()) {
            ...
            NotifyTransactionChanged( this, wtx.GetTxHash(), CT_UPDATED ) ;
            ...
            // available of the outputs it spends. Then recompute those
            for ( const CTxIn & txin : wtx.tx->vin )
            {
                if (mapWallet.count(txin.prevout.hash))
 ...
 *
 * If fUpdate is true, found transactions that already exist in the wallet
 * will be updated
 *
...
std::vector< uint256 > CWallet::ResendWalletTransactionsBefore( int64_t nTime, CConnman* connman )
{
    ...
    for ( std::pair< const unsigned int, CWalletTx* > & item : mapSorted )
    {
        ...
        if ( wtx.RelayWalletTransaction( connman ) )
            result.push_back( wtx.GetTxHash() ) ;
    }
...
CAmount CWallet::GetImmatureBalance() const
{
    CAmount balance = 0 ;
    {
        ...
        {
            const CWalletTx * pcoin = &item.second ;
            balance += pcoin->GetImmatureCredit() ;
        }
    }
...
CAmount CWallet::GetImmatureWatchOnlyBalance() const
{
    CAmount balance = 0 ;
    {
        ...
        {
            const CWalletTx * pcoin = &item.second ;
            balance += pcoin->GetImmatureWatchOnlyCredit() ;
        }
    }
                              ...
                              bool fIncludeZeroValue ) const
{
    ...
    {
        LOCK2( cs_main, cs_wallet ) ;
        for ( const std::pair< const uint256, CWalletTx > & entry : mapWallet )
        {
            ...
            // Don't consider coins which aren't at least in the mempool
...
bool CWallet::SelectCoinsMinConf( const CAmount & nTargetValue,
                                  const int nConfMine, const int nConfTheirs, const uint64_t nMaxAncestors,
                                  ...
                                  CAmount & nValueRet ) const
{
    ...
    for ( const COutput & output : vCoins )
    {
        ...
        if ( ! mempool.TransactionWithinChainLimit( pcoin->GetTxHash(), nMaxAncestors ) )
            continue ;
    ...
    // If we have a bigger coin and (either the stochastic approximation
    // didn't find a good solution, or the next bigger coin is closer),
                           ...
                           const CCoinControl * coinControl ) const
{
    ...
    if ( coinControl != nullptr && coinControl->HasSelected() && ! coinControl->fAllowOtherInputs )
    {
    ...
    {
        if ( setPresetCoins.count( std::make_pair( it->tx, it->i ) ) > 0 )
            it = vCoins.erase( it ) ;
        else
            ++ it ;
    }
                                 ...
                                 bool sign )
{
    ...
    assert( txNew.nLockTime <= static_cast< unsigned int >( chainActive.Height() ) ) ;
...

    {
        ...
        LOCK2( cs_main, cs_wallet ) ;
        {
            ...
            nFeeRet = 0 ;
            // begin with no fee and loop until there is enough fee
            while ( true )
            {
                ...
                for ( const auto & coin : setCoins )
                    txNew.vin.push_back( CTxIn( coin.first->GetTxHash(), coin.second, CScript(),
                ...
                if ( nFeeRet >= nFeeNeeded ) {
                    ...
                    // to be addressed to avoid creating too small output
...
/**
 * Call after CreateTransaction unless you want to cancel
 */
...
{
    {
        ...
        LogPrintf( "CWallet::%s\n%s", __func__, wtxNew.tx->ToString() ) ;
        {
            ...
            for ( const CTxIn & txin : wtxNew.tx->vin )
            {
                ...
                NotifyTransactionChanged( this, coin.GetTxHash(), CT_UPDATED ) ;
            }
        }

        // Track how many getdata requests our transaction gets
        mapRequestCount[ wtxNew.GetTxHash() ] = 0 ;
...
bool CWallet::SetAddressBook( const CTxDestination & address, const std::string & strName, const std::string & strPurpose )
{
    ...
    if ( ! fFileBacked )
        return false ;
    if ( ! strPurpose.empty() && ! CWalletDB( strWalletFile ).WritePurpose( CBase58Address( address ).ToString(), strPurpose ) )
        return false ;

    return CWalletDB( strWalletFile ).WriteName( CBase58Address( address ).ToString(), strName ) ;
}
...
{
    {
        ...
        if ( fFileBacked )
        {
            ...
            std::string strAddress = CBase58Address( address ).ToString() ;
    ...
    if ( ! fFileBacked )
        return false ;
    CWalletDB( strWalletFile ).ErasePurpose( CBase58Address( address ).ToString() ) ;
    return CWalletDB( strWalletFile ).EraseName( CBase58Address( address ).ToString() ) ;
}
...
{
    {
        ...
        int64_t nKeys = std::max( GetArg( "-keypool", DEFAULT_KEYPOOL_SIZE ), static_cast< int64_t >( 0 ) ) ;
...
{
    {
        ...
        // Top up key pool
        unsigned int nTargetSize = 0 ;
        if ( kpSize > 0 )
            nTargetSize = kpSize ;
        else {
            uint64_t keypoolSizeFromArg = GetArg( "-keypool", DEFAULT_KEYPOOL_SIZE ) ;
            if ( keypoolSizeFromArg > 0 )
                nTargetSize = keypoolSizeFromArg ;
        }
...
std::map< CTxDestination, CAmount > CWallet::GetAddressBalances()
{
    ...
    {
        LOCK( cs_wallet ) ;
        for ( std::pair< const uint256, CWalletTx > & walletEntry : mapWallet )
        {
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if ( ! fFileBacked )
        return true ;
    return CWalletDB( strWalletFile ).WriteDestData( CBase58Address( dest ).ToString(), key, value ) ;
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if ( ! fFileBacked )
        return true ;
    return CWalletDB( strWalletFile ).EraseDestData( CBase58Address( dest ).ToString(), key ) ;
}
