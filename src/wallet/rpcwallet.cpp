...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    return CBase58Address( keyID ).ToString() ;
}


CBase58Address GetAccountAddress( std::string strAccount, bool bForceNew = false )
{
    CPubKey pubKey ;
    ...
    return CBase58Address( pubKey.GetID() ) ;
}
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    for ( const std::pair< CBase58Address, CAddressBookData > & item : pwalletMain->mapAddressBook )
    {
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp )
        throw std::runtime_error(
            ...
            "      amount,                 (numeric) The amount in " + NameOfE8Currency() + "\n"
    ...
    for ( std::set< CTxDestination > grouping : pwalletMain->GetAddressGroupings() )
    {
        ...
        for ( CTxDestination address : grouping )
        {
            ...
            addressInfo.push_back( CBase58Address( address ).ToString() ) ;
            addressInfo.push_back( ValueFromAmount( balances[ address ] ) ) ;
            {
                if ( pwalletMain->mapAddressBook.find( CBase58Address( address ).Get() ) != pwalletMain->mapAddressBook.end() )
                    addressInfo.push_back( pwalletMain->mapAddressBook.find( CBase58Address( address ).Get() )->second.name ) ;
            }
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp || request.params.size() < 1 || request.params.size() > 2 )
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount   (numeric) The total amount in " + NameOfE8Currency() + " received at this address.\n"
            "\nExamples:\n"
    ...
    // Dogecoin address
    CBase58Address address = CBase58Address( request.params[ 0 ].get_str() ) ;
    if ( ! address.IsValid() )
        throw JSONRPCError( RPC_INVALID_ADDRESS_OR_KEY, "Invalid Dogecoin address" ) ;
...
UniValue getreceivedbyaccount(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp || request.params.size() < 1 || request.params.size() > 2 )
        throw std::runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount              (numeric) The total amount in " + NameOfE8Currency() + " received for this account.\n"
            "\nExamples:\n"
...
UniValue getbalance(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp || request.params.size() > 3 )
        throw std::runtime_error(
            ...
            "                     or even negative balances. In common, account balance calculation\n"
            "                     is not considered reliable and has resulted in confusing outcomes, so\n"
            "                     it is recommended to avoid passing this argument\n"
            "2. minconf           (numeric, optional, default=1) Only include transactions confirmed at least this many times\n"
...
UniValue movecmd(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp || request.params.size() < 3 || request.params.size() > 5 )
        throw std::runtime_error(
            ...
            "3. amount            (numeric) Quantity of " + NameOfE8Currency() + " to move between accounts.\n"
            "4. (dummy)           (numeric, optional) Ignored. Remains for backward compatibility.\n"
            "5. \"comment\"       (string, optional) An optional comment, stored in the wallet only.\n"
            "\nResult:\n"
            "true|false           (boolean) true if successful.\n"
            "\nExamples:\n"
            "\nMove 0.01 " + NameOfE8Currency() + " from the default account to the account named tabby\n"
            + HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 " + NameOfE8Currency() + " timotei to akiko with a comment and funds have 6 confirmations\n"
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp || request.params.size() < 3 || request.params.size() > 6 )
        throw std::runtime_error(
            ...
            "3. amount                (numeric or string, required) The amount in " + NameOfE8Currency() + " (transaction fee is added on top).\n"
            ...
            "\"txid\"                 (string) The transaction id.\n"
            "\nExamples:\n"
            "\nSend 0.01 " + NameOfE8Currency() + " from the default account to the address, must have at least 1 confirmation\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp || request.params.size() < 2 || request.params.size() > 5 )
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The dogecoin address is the key, the numeric amount (can be string) in " + NameOfE8Currency() + " is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    {
        CBase58Address address( key ) ;
        if ( ! address.IsValid() )
            throw JSONRPCError( RPC_INVALID_ADDRESS_OR_KEY, std::string( "Invalid Dogecoin address: " ) + key ) ;
        ...
        CScript scriptPubKey = GetScriptForDestination(address.Get());
        CAmount nAmount = AmountFromValue( sendTo[ key ] ) ;
        if (nAmount <= 0)
            throw JSONRPCError(RPC_TYPE_ERROR, "Invalid amount for send");
        totalAmount += nAmount;
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    return CBase58Address( innerID ).ToString() ;
}
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    return CBase58Address( w.result ).ToString() ;
}
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    std::map< CBase58Address, tallyitem > mapTally ;
    ...
    for ( const std::pair< CBase58Address, CAddressBookData > & item : pwalletMain->mapAddressBook )
    {
        ...
        std::map< CBase58Address, tallyitem >::iterator it = mapTally.find( address ) ;
        ...
        if ( fByAccounts )
        {
            tallyitem & tallyItem = mapAccountTally[ strAccount ] ;
            tallyItem.nAmount += nAmount ;
            tallyItem.nConf = std::min( tallyItem.nConf, nConf ) ;
            tallyItem.fIsWatchonly = fIsWatchonly ;
        }
        else
        {
            ...
            if ( it != mapTally.end() ) {
                for ( const uint256 & txhash : ( *it ).second.txids )
...
UniValue listreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp || request.params.size() > 3 )
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in " + NameOfE8Currency() + " received by the address\n"
...
void ListTransactions( const CWalletTx & wtx, const std::string & strAccount, int nMinDepth, bool fLong, UniValue & ret, const isminefilter & filter )
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        for ( const COutputEntry & s : listSent )
        {
            ...
            if ( pwalletMain->mapAddressBook.count( s.destination ) > 0 )
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp || request.params.size() > 4 )
        throw std::runtime_error(
            ...
            "    \"amount\": x.xxx,          (numeric) The amount in " + NameOfE8Currency() + ". This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"label\": \"label\",       (string) A comment for the address/transaction, if any\n"
            "    \"vout\": n,                (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in " + NameOfE8Currency() + ". This is negative and only available for the \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp )
        throw std::runtime_error(
            ...
            "    \"amount\": x.xxx,          (numeric) The amount in " + NameOfE8Currency() + ". This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in " + NameOfE8Currency() + ". This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp || request.params.size() < 1 || request.params.size() > 2 )
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in " + NameOfE8Currency() + "\n"
            "  \"fee\": x.xxx,            (numeric) The amount of the fee in " + NameOfE8Currency() + ". This is negative and only available for the \n"
            "                              'send' category of transactions.\n"
            ...
            "      \"amount\" : x.xxx,                 (numeric) The amount in " + NameOfE8Currency() + "\n"
            "      \"label\" : \"label\",              (string) A comment for the address/transaction, if any\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "      \"fee\": x.xxx,                     (numeric) The amount of the fee in " + NameOfE8Currency() + ". This is negative and only available for the \n"
            "                                           'send' category of transactions.\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp || request.params.size() != 0 )
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total confirmed balance of the wallet in " + NameOfE8Currency() + "\n"
            "  \"unconfirmed_balance\": xxx,   (numeric) the total unconfirmed balance of the wallet in " + NameOfE8Currency() + "\n"
            "  \"immature_balance\": xxxxxx,   (numeric) the total immature balance of the wallet in " + NameOfE8Currency() + "\n"
...
UniValue resendwallettransactions(const JSONRPCRequest& request)
{
    ...
    for ( const uint256 & txhash : txids )
        result.push_back( txhash.ToString() ) ;
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp || request.params.size() > 4 )
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,         (numeric) the transaction output amount in " + NameOfE8Currency() + "\n"
            "    \"confirmations\" : n,      (numeric) The number of confirmations\n"
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CBase58Address address( input.get_str() ) ;
            if ( ! address.IsValid() )
    ...
    for ( const COutput & out : vecOutputs )
    {
        ...
        UniValue entry( UniValue::VOBJ ) ;
        entry.pushKV( "txid", out.tx->GetTxHash().GetHex() ) ;
        entry.pushKV( "vout", out.i ) ;

        if ( fValidAddress ) {
            entry.pushKV( "address", CBase58Address( address ).ToString() ) ;
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if ( request.fHelp || request.params.size() < 1 || request.params.size() > 2 )
        throw std::runtime_error(
                            ...
                            "     \"feeRate\"                (numeric, optional, default not set: makes wallet determine the fee) Set a specific feerate (" + NameOfE8Currency() + " per KB)\n"
                            ...
                            "  \"fee\":       n,         (numeric) Fee in " + NameOfE8Currency() + " the resulting transaction pays\n"
    ...
    if (request.params.size() > 1) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if ( options.exists( "changeAddress" ) ) {
            CBase58Address address( options[ "changeAddress" ].get_str() ) ;
